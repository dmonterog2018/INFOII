import socket, sys, threading, time, pickle

from cola import *
from check_inicio import inicio
from listas_enlazadas import *

max_partidas = int(sys.argv[2]) # Pasamos el numero maximo de partidas por argumento
ranking = str(sys.argv[3]) # Pasamos el fichero por argumento
print("Arrancando servidor de juego...")
print("IP servidor de juego: ", socket.gethostbyname(socket.gethostname()))
server_socket = socket.socket(socket.AF_INET, socket.SOCK_STREAM)
server_socket.bind((socket.gethostname(), int(sys.argv[1]))) # Pasamos puerto por argumento
...
def metodo_ranking():
    with open(ranking, 'r') as f: # Leemos el fichero
...
def bienvenida(client_socket, usuario):
    if max_partidas > contador_partidas:
        if p.vacia() == True:  # Enviamos el mensaje de sala de espera si solo hay un cliente conectado
...
def manejo_partida(j1, j2):
    ...
    final = False # Iniciamos el final en false para que no se salga del bucle hasta que finalize
...
def mensajeria_turno(jugador1, j2, contador_turnos):
    ...
    if pickle.loads(datos5) == 'FIN': # Hemos movido unicamente por lo que , pasamos el turno al sigueinte.
        ...
        puntos_ganador = (pickle.loads(datos2)[2])
        ...
        puntos_perdedor = int(pickle.loads(datos)[1])
        if puntos_perdedor > puntos_ganador: # Comprobamos que perdedor no tenga mas puntos que ganador
            puntos_ganador = 1000
            puntos_perdedor = 900

        print(puntuacionganador[0], puntos_ganador)
        print(puntuacionperdido[0], puntos_perdedor)
        l.insertarEn(puntuacionganador[0], int(puntos_ganador))
        time.sleep(1)
        l.insertarEn(puntuacionperdido[0], int(puntos_perdedor))
        time.sleep(1)
    ...
    else: # Si realizamos una accion y recibimos una tupla la enviamos al oponente, y esperamos a recibir el resultado de la accion y continuamos con el siguiente turno.
