...
class medico(Personajes):
    def __init__(self, vida_maxima, vida_actual, dano, posicion, enfriamiento):
        super().__init__(vida_maxima, vida_actual, dano, posicion, enfriamiento) #Inicializar los parametros de la Clase Base
    ...
    def habilidad(self, celda_curar, equipo):
        if self.vida_actual > 0: #Comprobar que el medico esta vivo
            if validar_celda(celda_curar,'d', 4): #Validar que la celda que intentamos curar esta dentro del tablero
                if comprobar_celda_disponible(celda_curar, equipo) == False: #Comprobamos que la celda esta ocupada por un miembre de nuestro equipo
                    for miembro in equipo: #Recorremos la lista equipo
                        for validar in miembro.values(): #Recorremos los valores del diccionario de la lista equipo
                            if validar == celda_curar: #Comprobamos si el valor actual de validar es igual a celda_curar 
                                if miembro['vida actual'] < miembro['vida maxima'] and miembro['vida actual'] > 0: #Comprobamos que el personaje que queremos curar esta vivo y su vida es menor a su vida maxima
                                    miembro['vida actual'] = miembro['vida maxima'] #Si la condicion se cumple, igualamos su vida actual al valor de vida_maxima
                                    return True
    ...
    def mover(self, celda_nueva, equipo):
        if validar_celda(celda_nueva, 'd', 4): #Validar que la celda a la que intentamos movernos esta dentro del tablero
            if comprobar_celda_disponible(celda_nueva, equipo): #Comprobamos que la celda no esta ocupada
                if validar_celda_contigua(self.posicion, celda_nueva): #Comprobamosque la celda a la que intentamos movernos es adyacente
    ...
    def habilidad(self,celda_atacar, equipo):
        if equipo[2]['vida actual'] > 0: #Comprobar que el personaje esta vivo
            if equipo[2]['enfriamiento'] == 0: #Comrpobar que la habilidad no esta en enfriamiento
                if validar_celda(celda_atacar, 'd', 4): #Validar que la celda que intentamos atacar esta dentro del tablero
